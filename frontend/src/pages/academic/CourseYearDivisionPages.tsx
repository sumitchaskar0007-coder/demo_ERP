  ...
  return (
    <div className="page-container">
      ...
      <Card className="mt-6 overflow-hidden">
        <div className="filter-grid sm:grid-cols-2 xl:grid-cols-4">
          <Input
...
export function CourseYearFormPage() {
  ...
  const submit = async (v: CourseYearValues) => {
    try {
      if (editId)
        await academicApi.updateCourseYear(editId, { displayName: v.displayName, code: v.code });
      else await academicApi.createCourseYear(v);
...
export function DivisionListPage() {
  ...
  const [page, setPage] = useState(0);
  const [loading, setLoading] = useState(true);
  const [assigning, setAssigning] = useState<Division | null>(null);
  ...
  }, [courseYearId, departmentId, keyword, page, status]);
  useEffect(() => {
    const t = setTimeout(load, 250);
    return () => clearTimeout(t);
  }, [load]);
  const openAssign = async (row: Division) => {
    setAssigning(row);
    setTeacherId("");
    try {
      setTeachers(await academicApi.eligibleClassTeachers(row.id));
    } catch (e) {
      toast.error(handleApiError(e).message);
    }
  };
  const assign = async () => {
    if (!assigning || !teacherId) return;
    try {
      await academicApi.assignDivisionClassTeacher(assigning.id, Number(teacherId));
  ...
  const toggle = async (r: Division) => {
    try {
      await academicApi.setDivisionStatus(r.id, r.status !== "ACTIVE");
      await load();
    } catch (e) {
      toast.error(handleApiError(e).message);
    }
  };
  const remove = async (r: Division) => {
    try {
      await academicApi.removeDivisionClassTeacher(r.id);
  ...
  const columns: Column<Division>[] = [
    ...
    { key: "division", header: "Division", render: (r) => `${r.name} (${r.code})` },
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage divisions and their Class Teacher assignments.</p>
        </div>
        <div className="page-header__actions">
        ...
        <div className="filter-grid sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Select
            ...
            options={[
              ...
              ...years.map((y) => ({ label: y.displayName, value: y.id })),
            ]}
            value={courseYearId}
            onChange={(e) => {
      ...
      {assigning && (
        <Card className="mt-6 p-6">
          <h2 className="font-bold">Assign Class Teacher</h2>
          <p className="mt-1 text-sm text-slate-500">
            {assigning.departmentName} → {assigning.courseYearDisplayName} → {assigning.name}
          </p>
          <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-end">
            <Select
              ...
              options={[
                ...
                ...teachers.map((t) => ({
                  label: `${t.fullName} (${t.staffType.replaceAll("_", " ")})`,
                  value: t.id,
                })),
              ]}
              value={teacherId}
            ...
            />
            <Button className="w-full sm:w-auto" onClick={assign} disabled={!teacherId}>
              Assign
            </Button>
            <Button className="w-full sm:w-auto" variant="secondary" onClick={() => setAssigning(null)}>
              Cancel
            </Button>
          </div>
        </Card>
...
export function DivisionFormPage() {
  ...
  const submit = async (v: DivisionValues) => {
    try {
      if (editId)
  ...
  return (
    <div className="page-container">
      ...
      <p className="page-subtitle">Choose an active Course Year inside the Department.</p>
      <Card className="mt-6 p-6">
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit(submit)}>
          <Select
            label="Department"
          ...
          <Select
            ...
            disabled={Boolean(editId)}
            options={[
              { label: "Select Course Year", value: "" },
              ...years.map((y) => ({ label: y.displayName, value: y.id })),
            ]}
