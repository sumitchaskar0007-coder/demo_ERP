...
import {
  exportWeeklyTimetableExcel,
  exportWeeklyTimetablePdf,
} from "@/features/academics/weeklyTimetableExport";
...
export function CourseYearListPage() {
  ...
  useEffect(() => {
    academicApi
      .searchCourseYears({ departmentId: departmentId || undefined, page: 0, size: 100 })
      .then((response) =>
        setAvailableYears([
          ...new Map(response.content.map((row) => [row.yearName, row])).values(),
        ]),
      )
  ...
  const load = useCallback(async () => {
    ...
    try {
      setResult(
        await academicApi.searchCourseYears({
    ...
    { key: "divisions", header: "Divisions", render: (r) => r.divisionCount },
    {
      ...
      render: (r) => (
        <div className="flex gap-2">
          <Link to={`/principal/course-years/${r.id}/edit`}>
  ...
  return (
    <div className="page-container">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="page-title">Course Years</h1>
          <p className="page-subtitle">Create each academic level inside a department.</p>
        </div>
        <Link to={ROUTES.createCourseYear}>
          <Button>Create Course Year</Button>
        </Link>
      </div>
      <Card className="mt-6">
        <div className="grid gap-3 border-b p-4 md:grid-cols-3">
          ...
          <Select
            ...
            options={[
              { label: departmentId ? "All Course Years" : "Select department first", value: "" },
              ...availableYears.map((year) => ({
                label: year.yearName.replaceAll("_", " "),
                value: year.yearName,
              })),
            ]}
            value={courseYearName}
            onChange={(e) => {
              setCourseYearName(e.target.value);
              setPage(0);
            }}
          />
          <Select
        ...
        {loading ? (
          <Loader label="Loading Course Years..." />
        ) : result.content.length ? (
          <>
            <DataTable columns={columns} data={result.content} rowKey={(r) => r.id} />
          ...
          <EmptyState
            title="No Course Years"
            description="Create the first Course Year for a department."
          />
        )}
      </Card>
    </div>
...
export function CourseYearFormPage() {
  ...
  const submit = async (values: CourseYearValues) => {
    try {
      if (editId)
        await academicApi.updateCourseYear(editId, {
  ...
  return (
    <div className="page-container">
      <h1 className="page-title">{editId ? "Edit" : "Create"} Course Year</h1>
      <p className="page-subtitle">
        Department and academic identity cannot be changed after creation.
      </p>
...
export function DivisionListPage() {
  ...
  useEffect(() => {
    academicApi
      ...
      .then((r) => setYears(uniqueById(r.content)))
      .catch(() => setYears([]));
  }, [departmentId]);
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setResult(
        await academicApi.searchDivisions({
          departmentId: departmentId || undefined,
  ...
  const toggle = async (row: Division) => {
    try {
      await academicApi.setDivisionStatus(row.id, row.status !== "ACTIVE");
      await load();
    } catch (error) {
      toast.error(handleApiError(error).message);
    }
  ...
  const columns: Column<Division>[] = [
    ...
    { key: "capacity", header: "Capacity", render: (r) => r.capacity },
    {
      ...
      render: (r) => r.classTeacherName || "Not assigned",
    },
    { key: "status", header: "Status", render: (r) => <StatusBadge status={r.status} /> },
    {
      key: "actions",
      ...
      render: (r) => (
        <div className="flex flex-wrap gap-2">
          <Link to={`/principal/divisions/${r.id}/edit`}>
            <Button variant="secondary">Edit</Button>
          </Link>
    ...
    <div className="page-container">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="page-subtitle">Browse and manage the divisions in your college.</p>
        </div>
      ...
      <Card className="mt-6">
        <div className="grid gap-3 border-b p-4 md:grid-cols-3">
          ...
          <Select
            ...
            options={[
              ...
              ...years.map((year) => ({
                label: `${year.displayName} (${year.academicYear})`,
                value: year.id,
              })),
            ]}
            value={courseYearId}
            onChange={(e) => {
              setCourseYearId(e.target.value);
              setPage(0);
            }}
          />
          <Select
        ...
        {loading ? (
          <Loader label="Loading Divisions..." />
        ) : result.content.length ? (
          <>
            <DataTable columns={columns} data={result.content} rowKey={(r) => r.id} />
            <div className="border-t p-4">
          ...
          <EmptyState
            title="No Divisions"
            description="Create a Division under an active Course Year."
          />
        )}
      </Card>
    </div>
...
export function DivisionFormPage() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors, isSubmitting },
  } = useForm<DivisionValues>({
    resolver: zodResolver(divisionSchema),
    defaultValues: { departmentId: 0, courseYearId: 0, name: "", code: "", capacity: 60 },
  });
  ...
  useEffect(() => {
    ...
    academicApi
      .searchCourseYears({ departmentId, status: "ACTIVE", page: 0, size: 100 })
      .then((r) => setYears(uniqueById(r.content)))
      .catch(() => setYears([]));
  }, [departmentId]);
  useEffect(() => {
  ...
  const submit = async (values: DivisionValues) => {
    try {
      if (editId)
        await academicApi.updateDivision(editId, {
  ...
  return (
    <div className="page-container">
      <h1 className="page-title">{editId ? "Edit" : "Create"} Division</h1>
      <p className="page-subtitle">
        {editId
          ? "Department and Course Year cannot be changed after creation."
          : "Choose a Department and one of its active Course Years."}
      </p>
      <Card className="mt-6 p-6">
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit(submit)}>
          {editId ? (
            <>
              <Input
                ...
                value={
                  editDetails
                    ? `${editDetails.departmentCode} - ${editDetails.departmentName}`
                    : "Loading..."
                }
              />
              <Input
                ...
                value={
                  editDetails
                    ? `${editDetails.courseYearDisplayName} (${editDetails.academicYear})`
                    : "Loading..."
                }
              />
            </>
            ...
            <>
              ...
              <Select
                ...
                options={[
                  {
                    label: departmentId ? "Select Course Year" : "Select department first",
                    value: "",
                  },
                  ...years.map((year) => ({
                    label: `${year.displayName} (${year.academicYear})`,
                ...
                {...register("courseYearId")}
              />
            </>
          )}
          <Input label="Division Name" error={errors.name?.message} {...register("name")} />
          <Input label="Division Code" error={errors.code?.message} {...register("code")} />
          <Input
            label="Capacity"
            type="number"
...
export function DivisionDetailsPage() {
  ...
  if (loading)
    return (
      <div className="page-container">
        <Loader label="Loading division details..." />
      </div>
    );
  if (!division)
    return (
      <div className="page-container">
        <EmptyState
          ...
          description="The requested division could not be loaded."
        />
      </div>
    );
  return (
    <div className="page-container">
      <Link
        to={ROUTES.divisions}
        className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-brand-600"
      >
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="page-subtitle">
            {division.departmentName} · {division.courseYearDisplayName} · {division.academicYear}
          </p>
        </div>
        <Link to={`/principal/divisions/${division.id}/edit`}>
          <Button variant="secondary">Edit Division</Button>
        </Link>
      </div>
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <Card className="p-5">
          ...
          <p className="mt-1 font-semibold text-slate-900">
            {division.classTeacherName || "Not assigned"}
          </p>
          <p className="text-sm text-slate-500">
            {division.classTeacherEmail || "No teacher assigned"}
          </p>
        </Card>
        <Card className="p-5">
        ...
        <Card className="p-5">
          ...
          <p className="mt-1 font-semibold text-slate-900">{division.courseYearDisplayName}</p>
      ...
      <Card className="mt-6 overflow-hidden">
        ...
        {students.length ? (
          ...
          <EmptyState
            ...
            description="Students assigned to this division will appear here."
          />
        )}
      </Card>
    </div>
...
function DivisionTimetable({ table }: { table: WeeklyTimetable | null }) {
  ...
  return (
    <section className="mt-6">
      ...
      {!table || !table.periods.length ? (
        <Card>
          <EmptyState
            title="No timetable available"
            description="The timetable for this division has not been configured yet."
          />
        </Card>
      ) : (
        <div className="overflow-x-auto rounded-2xl border bg-white shadow-sm">
          <div className="min-w-[1120px]">
            ...
            {table.periods.map((period) => (
              ...
              >
                <div
                  className={`sticky left-0 z-10 flex min-w-0 flex-col justify-center p-3 ${period.kind !== "TEACHING" ? "bg-amber-50" : "bg-white"}`}
                >
                ...
                {period.kind !== "TEACHING" ? (
                  ...
                  DAYS.map((day) => {
                    ...
                    return (
                      <div key={day} className="min-h-24 min-w-0 border-l p-2 text-left">
                        {entry ? (
                          <div
                            className={`h-full min-w-0 rounded-xl border p-2.5 shadow-sm ${TIMETABLE_COLORS[entry.subjectId % TIMETABLE_COLORS.length]}`}
                            title={`${entry.subject}\n${entry.teacher}\n${entry.room ?? "No room"}`}
                          >
                          ...
                          <div className="grid h-full min-h-20 place-items-center text-xs text-slate-300">
                            -
                          </div>
                        )}
                      </div>
