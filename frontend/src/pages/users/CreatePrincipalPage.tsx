...
import { createPrincipal } from "@/features/users/api";
...
export function CreatePrincipalPage() {
  ...
  const {
    ...
    defaultValues: { collegeId: 0, fullName: "", email: "", phone: "" },
  });
  useEffect(() => {
    getActiveColleges()
      .then(setColleges)
  ...
  const submit = async (values: FormValues) => {
    try {
      ...
      toast.success(`${response.message}. Account setup email has been queued.`);
  ...
  const options = [
    { label: "Select an active college", value: 0 },
        ...
        <Card className="p-6 sm:p-8">
          <div className="mb-7">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Assign one active Principal to an active college.
            </p>
          </div>
