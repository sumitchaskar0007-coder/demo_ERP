...
export function UserListPage() {
  ...
  const toggle = async () => {
    ...
    try {
      const response = await api.setUserStatus(confirming.id, confirming.status !== "ACTIVE");
  ...
  const columns: Column<User>[] = [
    ...
    { key: "phone", header: "Phone", render: (row) => row.phone || "—" },
    {
      key: "college",
      header: "College",
      render: (row) => row.collegeName || <span className="text-slate-400">System-wide</span>,
    },
    {
      key: "roles",
      header: "Roles",
    ...
    <div className="page-container">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="page-subtitle">Manage Super Admin and Principal accounts.</p>
        </div>
        <Button onClick={() => navigate(ROUTES.createPrincipal)}>
          <Plus className="h-4 w-4" />
          Create Principal
        </Button>
      </div>
      <Card className="mt-6">
        <div className="grid gap-3 border-b p-4 lg:grid-cols-[1fr_190px_180px_180px]">
          <Input
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Adjust your filters or create a Principal account."
          />
        )}
      </Card>
      <ConfirmDialog
        ...
        title={`${confirming?.status === "ACTIVE" ? "Deactivate" : "Activate"} user?`}
        description={`${confirming?.fullName || "This user"} will ${confirming?.status === "ACTIVE" ? "lose access to login" : "be able to log in again"}.`}
