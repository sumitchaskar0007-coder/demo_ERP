...
export function CreateStudentSectionStaffPage() {
  ...
  return (
    <div className="page-container">
      <div>
        <h1 className="page-title">Create Student Section Staff</h1>
        <p className="page-subtitle">Create a login account for admission verification staff.</p>
      </div>
        ...
        <Card className="p-6">
          <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 md:grid-cols-2">
            {admin ? (
              ...
              <Input label="College" value={user?.collegeName || ""} readOnly />
            )}
        ...
        <Card className="p-6">
          ...
          {created ? (
            <div className="mt-4 space-y-2 text-sm">
              <p>
                <b>Employee Code:</b> {created.employeeCode}
              </p>
              <p>
                <b>Name:</b> {created.fullName}
              </p>
              <p>
                <b>Email:</b> {created.email}
              </p>
              <p>
                <b>College:</b> {created.collegeName}
              </p>
              <p className="text-slate-500">Password is not shown after creation.</p>
            </div>
          ) : (
            <p className="mt-4 text-sm text-slate-500">
              Staff details will appear here after successful creation.
            </p>
          )}
        </Card>
      </div>
