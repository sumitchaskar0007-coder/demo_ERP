  ...
  return (
    <div className="page-container pb-10">
      <div className="mb-6">
        ...
        <h1 className="mt-2 text-2xl font-bold">Student Section Dashboard</h1>
        <p className="mt-1 text-sm text-slate-500">
          Review and manage college admission applications.
        </p>
      </div>
      <section className="erp-welcome-banner p-7 sm:p-9">
        <div className="absolute -right-12 -top-20 h-64 w-64 rounded-full border-[32px] border-blue-300/20" />
        <div className="relative flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
          <div>
            <p className="text-sm text-blue-100">{user?.collegeName || "Student Section"}</p>
            <h2 className="mt-2 text-3xl font-bold">
              Welcome back, {user?.fullName?.split(" ")[0]}
            </h2>
            <p className="mt-2 text-sm text-blue-100">
              Your admission verification workspace is ready.
            </p>
          </div>
          <Link
            to={ROUTES.studentSectionAdmissions}
            className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-white px-4 text-sm font-semibold text-brand-700 shadow-sm"
          >
          ...
          {cards.map(({ label, icon: Icon, color, accent }) => (
            <Card key={label} className="relative overflow-hidden p-5">
              ...
              <div className="flex items-start justify-between">
                ...
                <span className="rounded-full bg-slate-50 px-2 py-1 text-[10px] font-bold text-slate-400">
                  Live
                </span>
              </div>
              <p className="mt-4 text-2xl font-bold">{stats[label] ?? 0}</p>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
        <Card className="overflow-hidden">
          <div className="border-b px-6 py-5">
            <h2 className="font-bold">Admission pipeline</h2>
            <p className="mt-1 text-xs text-slate-500">
              Current application distribution by status
            </p>
          </div>
            ...
            {cards.slice(0, 4).map((card) => (
              <div key={card.label}>
                <div className="mb-2 flex justify-between text-sm">
                  ...
                  <span className="font-bold">{stats[card.label] ?? 0}</span>
                </div>
                <div className="h-2.5 overflow-hidden rounded-full bg-slate-100">
                  <div
                    className={`h-full rounded-full ${card.accent}`}
                    style={{
                      width: `${Math.max(5, ((stats[card.label] ?? 0) / Math.max(1, total)) * 100)}%`,
                    }}
                  />
                </div>
              </div>
        ...
        <Card className="overflow-hidden">
          <div className="border-b px-6 py-5">
            ...
            <p className="mt-1 text-xs text-slate-500">Circular overview of the workflow</p>
          </div>
          <div className="p-6">
            <DonutChart
          ...
          <div className="border-t px-6 py-4">
            ...
            >
              Open admission list <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </Card>
      </div>
