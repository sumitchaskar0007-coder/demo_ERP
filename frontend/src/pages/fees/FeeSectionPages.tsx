...
export function FeeSectionDashboardPage() {
  const [d, setD] = useState<FeeDashboardResponse | null>(null);
  useEffect(() => {
    api
      .getFeeDashboard()
      .then(setD)
  ...
  if (!d) return <div className="page-container">Loading dashboard…</div>;
  const items = [
    ["Fee Accounts", d.totalFeeAccounts],
    ["Pending Payments", d.pendingPayments],
    ["Verified Payments", d.verifiedPayments],
    ["Rejected Payments", d.rejectedPayments],
    ["Total Collected", `₹${d.totalCollectedAmount.toLocaleString("en-IN")}`],
    ["Total Pending", `₹${d.totalPendingAmount.toLocaleString("en-IN")}`],
    ["Today Collection", `₹${d.todayCollectedAmount.toLocaleString("en-IN")}`],
  ];
  return (
    <div className="page-container">
      <h1 className="page-title">Fee Section Dashboard</h1>
  ...
  const cols: Column<StudentFeeAccountResponse>[] = [
    {
      ...
      render: (r) => (
        <div>
          <b>{r.admissionNumber}</b>
          <p className="text-xs">{r.departmentName}</p>
        </div>
      ),
    },
...
export function FeeAccountDetailsPage() {
  ...
  useEffect(() => {
    if (feeAccountId)
      Promise.all([
        api.getFeeAccountById(+feeAccountId),
        api.getFeeAccountTransactions(+feeAccountId),
      ]).then(([x, y]) => {
        setA(x);
        setT(y);
      });
  ...
  return (
    <div className="page-container space-y-5">
      ...
      <Card className="p-6">
        ...
        {t.map((x) => (
          <div key={x.id} className="mt-3 flex justify-between border-t pt-3">
            <span>{x.transactionType.replaceAll("_", " ")}</span>
            <b>₹{x.amount}</b>
          </div>
        ))}
      </Card>
  ...
  const cols: Column<PaymentResponse>[] = [
    {
      ...
      render: (r) => (
        <div>
          <b>{r.studentName}</b>
          <p className="text-xs">{r.admissionNumber}</p>
        </div>
      ),
    },
    { key: "amount", header: "Amount", render: (r) => `₹${r.amount}` },
    { key: "reference", header: "Reference", render: (r) => r.transactionReference },
    { key: "mode", header: "Mode", render: (r) => r.paymentMode },
    { key: "status", header: "Status", render: (r) => <StatusBadge status={r.status} /> },
    {
      key: "action",
      header: "Action",
      render: (r) => (
        <Link to={`/fee-section/payments/${r.id}`}>
    ...
    <div className="page-container space-y-5">
      ...
      <Card className="p-4">
        <select
          ...
          value={s}
          onChange={(e) => setS(e.target.value as PaymentStatus | "")}
        >
...
export function PaymentDetailsPage() {
  ...
  const reject = () => {
    ...
    if (reason && reason.length >= 5)
      api.rejectPayment(p.id, reason).then(() => {
