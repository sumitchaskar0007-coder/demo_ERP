...
export interface AdmissionAnalyticsRow {
  id: number;
...
export interface AdmissionAnalytics {
  ...
  trend: { date: string; applications: number; approved: number; rejected: number }[];
  departments: AdmissionGroup[];
  years: AdmissionGroup[];
  divisions: AdmissionGroup[];
...
export const getReport = (type: string, params?: object) =>
  apiClient
    ...
    .then((r) => r.data.data);
export const getAdmissionAnalytics = (params?: object) =>
  apiClient
