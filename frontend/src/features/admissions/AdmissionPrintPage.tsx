...
import type { AdmissionPrintResponse } from "./types";
  ...
  return (
    <div className="page-container print-page-bg">
      <div className="no-print mb-4 flex flex-wrap gap-2">
        ...
        <Button variant="secondary" onClick={markPrinted}>Mark as Printed</Button>
      </div>
      ...
      <Card className="print-container admission-form-sheet mx-auto max-w-[820px] p-8 text-black">
        ...
        <div className="mt-3 grid grid-cols-[1fr_112px] gap-4">
          <div>
            ...
            <p className="mt-4 text-center text-sm">
              Form No. <Line value={data.admissionReferenceNumber} width="190px" /> / {String(currentYear).slice(2)} - {String(currentYear + 1).slice(2)}
            </p>
          </div>
        ...
        <section className="mt-2 text-[14px] leading-8">
          ...
          <NumberedLine number={2}>
            ...
            <span className="ml-auto flex items-center gap-2"><Check checked label="" /> <span className="text-xs">Tick in appropriate Box</span></span>
          </NumberedLine>
          ...
          <NumberedLine number={5}>APAAR ID: <Line width="250px" /></NumberedLine>
          <NumberedLine number={6}>
            Nationality: <Line value="Indian" width="220px" />
            Religion & Caste: <Line width="260px" />
          </NumberedLine>
          <NumberedLine number={7}>
            Applicant's Mobile no. & Email ID: <Line value={`${data.student.phone} / ${data.student.email}`} />
          </NumberedLine>
          ...
          <NumberedLine number={9}>
            ...
            <div className="ml-7 grid grid-cols-3 gap-3">
              <span>Pin: <Line value={data.student.pincode} /></span>
              <span>State: <Line value={data.student.state} /></span>
              <span>City: <Line value={data.student.city} /></span>
            </div>
            <div className="ml-7 grid grid-cols-2 gap-3">
              <span>Mobile no.: <Line value={data.student.phone} /></span>
              <span>Email: <Line value={data.student.email} /></span>
            </div>
          </NumberedLine>
          ...
          <NumberedLine number={12}>Seat No. of Qualifying Entrance Test: <BoxLine boxes={12} /></NumberedLine>
          <NumberedLine number={13}>Total Score in the Test (CET): Written: <Line width="520px" /></NumberedLine>
          <NumberedLine number={14}>Last Graduation College Name & Address: <Line value={data.academic.previousSchoolName} /></NumberedLine>
        </section>

        <div className="mt-14 grid grid-cols-[1fr_1fr_1.4fr] items-end gap-5 text-sm">
          <span>Date: <Line width="150px" /></span>
          <span>Place: <Line width="180px" /></span>
    ...
    <header className="grid grid-cols-[120px_1fr_170px] items-center gap-4 text-center">
      <div className="text-[11px] font-semibold uppercase">
        <div className="mx-auto mb-1 grid h-16 w-16 place-items-center rounded-full border border-black text-[10px]">
          {data.college.logoUrl ? <img src={data.college.logoUrl} alt="" className="h-14 w-14 object-contain" /> : "AIMS"}
        </div>
      ...
      <div>
        ...
        <p className="text-xs">{[data.college.address, data.college.city, data.college.state].filter(Boolean).join(", ")}</p>
        <p className="text-xs">Email: {data.college.contactEmail || "-"} | Phone: {data.college.contactPhone || "-"}</p>
      </div>
...
function DeclarationSection({ declarations }: { declarations: string[] }) {
  const items = declarations.length ? declarations : [
...
function UndertakingSection() {
  return (
    <section className="mt-12 text-[14px] leading-7">
      ...
      <ol className="list-decimal space-y-4 pl-8">
        <li>I undertake to observe full attendance as per the University/Institute Rules and failing which I am aware that my terms will not be granted.</li>
        <li>So long as I am a student of Institute, I will do nothing either inside or outside the Institute which may result in disciplinary action against me under the Rules, Act and Laws.</li>
        <li>I agree and undertake that if the fees and other charges decided by the Institute are more than the current academic year fees, then I will pay the difference to the Institute on demand.</li>
      </ol>
      <div className="mt-16 grid grid-cols-2 gap-8">
        <span>Date: <Line width="160px" /></span>
        <b className="text-right">Signature of Applicant</b>
        <span>Place: <Line width="160px" /></span>
...
function DocumentChecklist() {
  return (
    <section className="mt-10 text-[13px]">
      <h3 className="mb-4 text-base font-extrabold">Documents Original ( for Verification ) & Set of attested xerox copies ( Attached with form )</h3>
      <div className="grid grid-cols-2 gap-x-10 gap-y-2">
        {documents.map((document, index) => (
          <div key={document} className="grid grid-cols-[1fr_22px] items-center gap-3">
            <span>{String(index + 1).padStart(2, "0")}.{document}</span>
...
function AcademicTable({ data }: { data: AdmissionPrintResponse }) {
  const rows = [
    ["10th", "", "", "", ""],
    [data.academic.previousClassName || "12th / Graduation", data.academic.previousSchoolName || "", "", "", data.academic.previousPercentage ?? ""],
    ["", "", "", "", ""],
    ["", "", "", "", ""],
  ];
  return (
    <table className="mt-2 w-full border-collapse text-center text-[13px] leading-5">
      <thead>
        <tr>
          {["Qualification", "School/College/Institute", "Board/University", "Year of Passing", "Marks Obtained (%)"].map((head) => (
            <th key={head} className="border border-black px-2 py-2 font-semibold">{head}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {rows.map((row, index) => (
          <tr key={index}>
            {row.map((cell, cellIndex) => <td key={cellIndex} className="h-8 border border-black px-2">{cell}</td>)}
          </tr>
        ))}
      </tbody>
    </table>
...
function CourseBox({ children }: { children: ReactNode }) {
  return <span className="min-w-24 border border-black px-4 py-1 text-center text-xl font-bold uppercase">{children}</span>;
}

function NumberedLine({ number, children }: { number: number; children: ReactNode }) {
  return <div className="grid grid-cols-[28px_1fr] gap-1"><b>{number}.</b><div>{children}</div></div>;
}

function Line({ value, width = "100%" }: { value?: unknown; width?: string }) {
  return <span className="inline-block border-b border-black px-1 align-baseline" style={{ minWidth: width }}>{value ? String(value) : "\u00a0"}</span>;
}

function Check({ label, checked }: { label: string; checked?: boolean }) {
  return <span className="ml-5 inline-flex items-center gap-2">{label && <span>{label}</span>}<span className="grid h-5 w-8 place-items-center border border-black text-xl leading-none">{checked ? "/" : ""}</span></span>;
}

function BoxLine({ boxes }: { boxes: number }) {
  return <span className="inline-flex align-middle">{Array.from({ length: boxes }).map((_, index) => <span key={index} className="h-6 w-8 border border-black" />)}</span>;
}
