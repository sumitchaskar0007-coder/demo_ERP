...
import type { StudentSectionAdmissionResponse } from "./types";
...
export function PrincipalReviewQueuePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setResult(await api.getPrincipalReviewReadyAdmissions({ keyword: keyword || undefined, page, size: PAGE_SIZE })); }
  ...
  const columns: Column<StudentSectionAdmissionResponse>[] = [
    ...
    { key: "status", header: "Status", render: (row) => <div className="space-y-1"><AdmissionStatusBadge status={row.status} /><Badge tone="warning">Ready for Principal Review</Badge></div> },
    { key: "verified", header: "Verified At", render: (row) => formatDate(row.studentSectionVerifiedAt) },
    { key: "print", header: "Prints", render: (row) => row.printCount || 0 },
    { key: "actions", header: "", render: (row) => <Button variant="secondary" onClick={() => navigate(`/principal/admissions/${row.id}`)}><Eye className="h-4 w-4" />View</Button> },
  ];
  return (
