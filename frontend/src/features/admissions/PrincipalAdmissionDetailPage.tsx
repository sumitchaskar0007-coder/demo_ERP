...
export function PrincipalAdmissionDetailPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [detail, timeline] = await Promise.all([api.getPrincipalAdmission(id), api.getPrincipalAdmissionHistory(id)]);
  ...
  return (
    <div className="page-container space-y-5">
      ...
      <DetailSection title="Student and Parent" rows={[["Email", admission.email], ["Phone", admission.phone], ["Parent", admission.parentName], ["Parent Phone", admission.parentPhone], ["Previous School", admission.previousSchoolName], ["Previous Percentage", admission.previousPercentage]]} />
      <DetailSection title="Student Section Verification and Print" rows={[["Verified At", formatDate(admission.studentSectionVerifiedAt)], ["Verified By", admission.studentSectionVerifiedByName], ["Remarks", admission.studentSectionRemarks], ["Print Count", admission.printCount], ["Last Printed", formatDate(admission.lastPrintedAt)], ["Last Printed By", admission.lastPrintedByName]]} />
