...
export function AdminStudentListPage() {
  ...
  useEffect(() => {
    getActiveColleges().then(setColleges).catch(() => setColleges([]));
  }, []);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      setResult(await api.searchStudents({
  ...
  const columns: Column<StudentProfileResponse>[] = [
    ...
    { key: "department", header: "Department", render: (row) => <div><p>{row.departmentName}</p><Badge>{row.departmentCode}</Badge></div> },
    { key: "status", header: "Status", render: (row) => <Badge tone={row.status.includes("REJECTED") ? "danger" : row.status === "UNDER_REVIEW" ? "warning" : "success"}>{row.status.replaceAll("_", " ")}</Badge> },
    ...
    <div className="page-container">
      ...
      <Card className="mt-6">
        <div className="grid gap-3 border-b p-4 lg:grid-cols-[1fr_220px_240px]">
          ...
          />
          <Select
            options={[{ label: "All colleges", value: "" }, ...colleges.map((college) => ({ label: college.name, value: college.id }))]}
            value={collegeId}
            onChange={(event) => { setCollegeId(event.target.value ? Number(event.target.value) : ""); setPage(0); }}
        ...
        {loading ? (
          ...
          <EmptyState title="No students found" description="Submitted student admissions will appear here." />
        )}
      </Card>
    </div>
  );
