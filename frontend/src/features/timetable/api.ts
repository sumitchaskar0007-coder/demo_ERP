...
export const timetableApi = {
  ...
  publish: async (id: number) => (await apiClient.post<Timetable>(`${BASE}/${id}/publish`)).data,

  archive: async (id: number) => (await apiClient.post<Timetable>(`${BASE}/${id}/archive`)).data,
  ...
  rooms: async () => (await apiClient.get<RoomInfo[]>(`${BASE}/rooms`)).data,
};

export const classTeacherApi = {
  mySection: async () => (await apiClient.get<SectionInfo>(`${CT}/my-section`)).data,

  subjects: async () => (await apiClient.get<SubjectInfo[]>(`${CT}/subjects`)).data,

  teachers: async (subjectId: number) =>
    (await apiClient.get<TeacherInfo[]>(`${CT}/teachers`, { params: { subjectId } })).data,

  periods: async () => (await apiClient.get<PeriodInfo[]>(`${CT}/periods`)).data,
};
