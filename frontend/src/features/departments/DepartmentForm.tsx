...
export function DepartmentForm({ department, colleges, user, onSubmit, onCancel }: {
  ...
  useEffect(() => {
    reset(department ? {
      collegeId: department.collegeId, name: department.name,
      code: department.code, description: department.description || "",
    } : { collegeId: user.collegeId || 0, name: "", code: "", description: "" });
  }, [department, reset, user.collegeId]);
  const options = [{ label: "Select college", value: 0 }, ...colleges.map((college) => ({ label: `${college.name} (${college.code})`, value: college.id }))];
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      {principal ? (
        <Input label="College" value={`${user.collegeName} (${user.collegeCode})`} disabled />
      ) : (
        <Select label="College *" options={options} disabled={editing} error={errors.collegeId?.message} {...register("collegeId")} />
      )}
      <div className="grid gap-4 sm:grid-cols-2">
        <Input label="Department name *" error={errors.name?.message} {...register("name")} />
        <Input label="Department code *" disabled={editing} error={errors.code?.message} {...register("code")} />
      </div>
      <Textarea label="Description" error={errors.description?.message} {...register("description")} />
      <div className="flex justify-end gap-3 border-t pt-5"><Button type="button" variant="secondary" onClick={onCancel}>Cancel</Button><Button type="submit" loading={isSubmitting}>{editing ? "Save changes" : "Create department"}</Button></div>
    </form>
  );
}
