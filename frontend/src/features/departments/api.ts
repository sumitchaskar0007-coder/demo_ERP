...
export async function searchDepartments(params: DepartmentSearchParams) {
  const { data } = await apiClient.get<ApiResponse<PageResponse<Department>>>(`${BASE}/search`, { params });
  return data.data;
}
...
export async function createDepartment(values: DepartmentFormValues & { collegeId: number; code: string }) {
  const { data } = await apiClient.post<ApiResponse<Department>>(BASE, values);
  return data;
}
export async function updateDepartment(id: number, values: Pick<DepartmentFormValues, "name" | "description">) {
...
export async function setDepartmentStatus(id: number, active: boolean) {
  const { data } = await apiClient.patch<ApiResponse<Department>>(`${BASE}/${id}/${active ? "activate" : "deactivate"}`);
  return data;
}
