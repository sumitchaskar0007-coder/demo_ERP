...
export function AttendanceReportPage() {
  ...
  const [selected, setSelected] = useState<StudentAnalyticsRow>();
  const [trendMode, setTrendMode] = useState<"daily" | "weekly" | "monthly">("daily");
  const [studentSearch, setStudentSearch] = useState("");
  const [gender, setGender] = useState("");
  const [studentSort, setStudentSort] = useState("lowest");
    ...
    () =>
      (data?.students ?? []).filter(
        (s) =>
          ...
          (!applied.range ||
            (applied.range === "below75"
              ? s.total > 0 && s.percentage < 75
              : s.total > 0 && s.percentage < 50)),
      ),
    [data, applied],
  );
  const rows = useMemo(
  ...
  const absentToday = new Set(
    students
      .filter((s) => s.history.some((h) => h.date === today && h.status === "ABSENT"))
      .map((s) => s.studentId),
  ).size;
    ...
    <div className="page-container space-y-6 pb-12 print:bg-white">
      <header className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          ...
          <h1 className="page-title mt-1">Attendance Analytics Dashboard</h1>
      ...
      {loading && !data ? (
        ...
        <ErrorState message={error} retry={() => load(applied)} />
      ) : data ? (
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            <Metric
              icon={TrendingUp}
              label="Overall Student Attendance"
              value={`${overall}%`}
              subtitle={`${attendedRecords.toLocaleString()} of ${totalRecords.toLocaleString()} records attended`}
              tone="blue"
            />
            <Metric
              icon={Users}
              ...
              subtitle="Distinct active students"
              tone="indigo"
            />
            <Metric
              icon={CalendarDays}
            ...
            <Metric
              ...
              subtitle="Requires submission"
              tone="orange"
            />
            <Metric
              icon={Building2}
              label="Departments"
              value={new Set(students.map((s) => s.departmentId)).size}
              subtitle="Departments in scope"
              tone="cyan"
            />
            <Metric
              icon={GraduationCap}
              label="Divisions"
              value={new Set(students.map((s) => s.divisionId)).size}
          ...
          <div className="grid gap-6 xl:grid-cols-3">
            <Card className="p-5 xl:col-span-2">
              <SectionTitle
                ...
                subtitle="Record-based attendance percentage over time"
                action={
                  <div className="flex rounded-lg bg-slate-100 p-1">
                    {(["daily", "weekly", "monthly"] as const).map((m) => (
                      <button
                        ...
                        className={`rounded-md px-3 py-1.5 text-xs font-semibold capitalize ${trendMode === m ? "bg-white text-brand-700 shadow-sm" : "text-slate-500"}`}
                      >
                        {m}
                      </button>
                    ))}
              ...
              <div className="mt-5 h-72">
                {trend.length ? (
                  ...
                  <Empty label="No trend data for this range" />
                )}
              </div>
            </Card>
            <Card className="p-5">
              <SectionTitle
                ...
                subtitle="Records, not unique student counts"
              />
              <div className="mt-5 h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
            <Card className="overflow-hidden">
              ...
              <AnalyticsTable
                students={students}
                operations={data.departmentOperations}
                drill={drill}
                setDrill={setDrill}
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            <Card className="p-5">
              <SectionTitle
                ...
                subtitle="Average attendance across active students"
              />
              <div className="mt-5 h-72">
                {deptStats.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                  ...
                  <Empty label="No department data" />
                )}
              </div>
            </Card>
            <Card className="p-5">
              <SectionTitle
                ...
                subtitle="Horizontal comparison; lowest divisions surface first"
              />
              <div className="mt-5 space-y-4">
                {[...divisionStats]
                  ...
                  .slice(0, 8)
                  .map((x) => (
                    <Progress key={x.key} label={x.label} value={x.percentage} />
                  ))}
              </div>
            </Card>
          </div>
          ...
          <LectureRegister
            ...
            setSearch={setRegisterSearch}
            page={registerPage}
            setPage={setRegisterPage}
            sort={registerSort}
            setSort={setRegisterSort}
          />
        </>
      ) : null}
  ...
  return (
    <label className="text-xs font-semibold text-slate-600">
      ...
      <input
        ...
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="mt-1.5 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
      />
    </label>
  );
}
...
function Metric({
  ...
  return (
    <Card className="group p-5 transition duration-200 hover:-translate-y-1 hover:shadow-md">
...
function SectionTitleWrap({ children }: { children: React.ReactNode }) {
  return <div className="border-b border-slate-100 p-5">{children}</div>;
}
function Legend({ c, label, value }: { c: string; label: string; value: number }) {
  return (
...
function Breadcrumb({ drill, setDrill }: { drill: Drill; setDrill: (d: Drill) => void }) {
  return (
    <div className="flex flex-wrap items-center gap-1 border-b bg-slate-50/70 px-5 py-4 text-sm">
      <button className="font-bold text-brand-700" onClick={() => setDrill({})}>
      ...
      {drill.department && (
        <>
          ...
          <button
            onClick={() =>
              setDrill({ departmentId: drill.departmentId, department: drill.department })
            }
      ...
      {drill.division && (
        <>
          <ChevronRight className="h-4 w-4 text-slate-400" />
          <span className="font-semibold">{drill.division}</span>
          <ChevronRight className="h-4 w-4 text-slate-400" />
...
function AnalyticsTable(p: {
  ...
  select: (s: StudentAnalyticsRow) => void;
}) {
  const scoped = p.students.filter(
    (s) =>
      (!p.drill.departmentId || s.departmentId === p.drill.departmentId) &&
      (!p.drill.year || s.year === p.drill.year) &&
      (!p.drill.divisionId || s.divisionId === p.drill.divisionId),
  );
  if (!p.drill.departmentId) {
    const rows = aggregate(
      ...
      ...r,
      pending: p.operations.find((o) => o.id === Number(r.key))?.pending ?? 0,
    }));
    return (
      <GroupTable
        ...
        rows={rows}
        columns="department"
        click={(r) => p.setDrill({ departmentId: Number(r.key), department: r.label })}
      />
    );
  }
  if (!p.drill.year)
    return (
      <GroupTable
        ...
        columns="year"
        click={(r) => p.setDrill({ ...p.drill, year: r.label })}
      />
    );
  if (!p.drill.divisionId)
    return (
      <GroupTable
        ...
        columns="division"
        click={(r) => p.setDrill({ ...p.drill, divisionId: Number(r.key), division: r.label })}
      />
    );
  let list = scoped
    .filter(
      (s) =>
        !p.search ||
        `${s.rollNumber} ${s.studentName}`.toLowerCase().includes(p.search.toLowerCase()),
    )
    .filter((s) => !p.gender || s.gender === p.gender);
  list = [...list].sort((a, b) =>
    p.sort === "highest"
    ...
    <div>
      <div className="flex flex-wrap items-end gap-3 border-b p-5">
        ...
        <Select
          ...
          options={unique(scoped.map((s) => s.gender || "Not specified"))}
          all="All genders"
        />
        <Select
          label="Sort"
      ...
      <div className="overflow-x-auto">
        ...
        {!list.length && <Empty label="No students match these filters" />}
      </div>
    </div>
  );
}
...
function GroupTable({
  ...
  title: string;
  rows: Agg[];
  columns: "department" | "year" | "division";
  click: (r: Agg) => void;
}) {
  ...
  return (
    <div>
      <div className="px-5 py-4">
        ...
        <p className="text-xs text-slate-500">Click a row to continue the drill-down.</p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[850px] text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase text-slate-500">
            <tr>
              ...
              {columns === "division" && <th className="px-4 py-3">Class Teacher</th>}
          ...
          <tbody>
            {rows.map((r) => (
              <tr
                key={r.key}
                onClick={() => click(r)}
                className="cursor-pointer border-t transition hover:bg-brand-50/40"
              >
                <td className="px-5 py-4 font-bold">{r.label}</td>
                {columns === "division" && <td className="px-4 py-4">{r.classTeacher}</td>}
                <td className="px-4 py-4">{r.students}</td>
                ...
                <td className="px-4 py-4 text-brand-700">
                  Drill down <ChevronRight className="inline h-4 w-4" />
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {!rows.length && <Empty label="No analytics available" />}
      </div>
    </div>
  );
}
function AttentionPanel({
  ...
  students: StudentAnalyticsRow[];
  onSelect: (s: StudentAnalyticsRow) => void;
}) {
  const low = [...students]
  ...
  return (
    <Card className="self-start p-5 xl:sticky xl:top-48">
      <SectionTitle
        ...
        subtitle={`${students.filter((s) => s.total > 0 && s.percentage < 75).length} below 75% · ${students.filter((s) => s.total > 0 && s.percentage < 60).length} below 60% · ${students.filter((s) => s.total > 0 && s.percentage < 50).length} below 50%`}
      />
      <div className="mt-4 space-y-2">
        {low.map((s) => (
          <button
            key={s.studentId}
            onClick={() => onSelect(s)}
            className="flex w-full items-center gap-3 rounded-xl border border-slate-100 p-3 text-left transition hover:border-rose-200 hover:bg-rose-50"
          >
            <Avatar name={s.studentName} />
            <span className="min-w-0 flex-1">
              <b className="block truncate text-sm">{s.studentName}</b>
              <small className="text-slate-500">
                {s.department} · {s.rollNumber || "No roll no."}
              </small>
            </span>
            <b className="text-sm text-rose-600">{s.percentage}%</b>
          </button>
        ))}
        {!low.length && <Empty label="No students below 75%" />}
      </div>
    </Card>
  );
}
    ...
    <Card className="overflow-hidden">
      ...
      <SimpleTable
        ...
        rows={grouped.map((x) => [
          ...
          x.lectures,
          x.submitted,
          <span className={x.pending ? "font-bold text-rose-600" : "text-slate-500"}>
            {x.pending}
          </span>,
          <Percent value={x.percentage} />,
        ])}
      />
    </Card>
  );
...
function SubjectAnalytics({
  ...
  const names = unique(students.flatMap((s) => s.subjects.map((x) => x.subject)));
  const items = names.map((name) => {
    const values = students
      .map((s) => s.subjects.find((x) => x.subject === name)?.percentage)
      .filter((x): x is number => x !== undefined);
    const teachers =
      unique(rows.filter((r) => r.subject === name).map((r) => r.teacher)).join(", ") || "—";
    const avg = values.length ? round(values.reduce((a, b) => a + b, 0) / values.length) : 0;
    return [
      name,
      <Percent value={avg} />,
    ...
    <Card className="overflow-hidden">
      <SectionTitleWrap>
        <SectionTitle
          ...
          subtitle="Average, highest and lowest student attendance"
        />
      </SectionTitleWrap>
      <SimpleTable
        heads={["Subject", "Average", "Highest", "Lowest", "Teacher", "Status"]}
        rows={items}
      />
    </Card>
  );
...
function SimpleTable({ heads, rows }: { heads: string[]; rows: React.ReactNode[][] }) {
  return (
    <div className="overflow-x-auto">
      ...
      {!rows.length && <Empty label="No records available" />}
    </div>
  );
}
...
function LectureRegister({
  ...
  let list = rows.filter(
    (r) =>
      !search ||
      `${r.subject} ${r.department} ${r.teacher} ${r.division}`
        .toLowerCase()
  ...
  return (
    <Card className="overflow-hidden">
      <div className="flex flex-wrap items-end justify-between gap-3 border-b p-5">
        <SectionTitle
          icon={BarChart3}
          title="Lecture register"
          subtitle="Submitted lecture-level attendance records"
        />
        <div className="flex gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="h-10 rounded-lg border pl-9 pr-3 text-sm"
            />
          </div>
          <select
            value={sort}
          ...
          >
            ...
            <option value="percentage">Lowest attendance</option>
          </select>
        </div>
      </div>
      <div className="max-h-[560px] overflow-auto">
            ...
            {shown.map((r) => (
              <tr key={r.id} className="border-t hover:bg-slate-50">
                <td className="px-4 py-3">
                  {r.date}
                  <br />
                  <small className="text-slate-400">{r.time}</small>
                </td>
        ...
        {!shown.length && <Empty label="No lecture sessions match the search" />}
      </div>
      <div className="flex items-center justify-between border-t px-5 py-4 text-sm">
        <span>
          {list.length} sessions · Page {Math.min(page, pages)} of {pages}
        </span>
        <div className="flex gap-2">
...
function StudentDrawer({ student, close }: { student: StudentAnalyticsRow; close: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/35" onMouseDown={close}>
      <aside
        onMouseDown={(e) => e.stopPropagation()}
        className="h-full w-full max-w-2xl overflow-y-auto bg-white text-slate-900 shadow-2xl"
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white/95 p-5 backdrop-blur">
          <div className="flex items-center gap-3">
            <Avatar name={student.studentName} large />
            <div>
              <h2 className="font-bold text-slate-900">{student.studentName}</h2>
              <p className="text-xs text-slate-500">
                PRN {student.admissionNumber} · Roll {student.rollNumber || "—"}
              </p>
            </div>
          </div>
        ...
        <div className="space-y-6 p-5">
          <div className="grid gap-3 sm:grid-cols-2">
            <Info label="Department / Course" value={student.department} />
            <Info label="Academic Year" value={student.academicYear} />
            <Info label="Year / Division" value={`${student.year} · ${student.division}`} />
            <Info label="Class Teacher" value={student.classTeacher} />
            <Info label="Guardian" value={student.guardianName || "—"} />
            <Info label="Mobile" value={student.mobile || "—"} />
          </div>
          <Card className="p-5">
            <p className="text-xs font-bold uppercase text-slate-500">Overall attendance</p>
          ...
          <div>
            <h3 className="mb-3 font-bold">Subject-wise attendance</h3>
            <div className="space-y-3">
              {student.subjects.map((s) => (
                <Progress key={s.subjectId} label={s.subject} value={s.percentage} />
              ))}
            </div>
          </div>
          <Card className="p-5">
          ...
          <div>
            <h3 className="mb-3 font-bold">Attendance history</h3>
            <SimpleTable
              heads={["Date", "Subject", "Teacher", "Status"]}
              rows={student.history.map((h) => [
                h.date,
                h.subject,
                h.teacher,
    ...
    value >= 95
      ...
      : value >= 85
        ...
        : value >= 75
          ? "text-amber-600"
          : "text-rose-600";
  return <b className={c}>{round(value)}%</b>;
}
...
function Empty({ label }: { label: string }) {
  return (
    <div className="grid min-h-28 place-items-center p-8 text-center text-sm text-slate-400">
      <div>
        <BarChart3 className="mx-auto mb-2 h-7 w-7 opacity-40" />
        {label}
      </div>
    </div>
  );
  ...
  return (
    <Card className="grid min-h-72 place-items-center p-8 text-center">
      <div>
        ...
        <h2 className="mt-3 font-bold">Unable to load attendance analytics</h2>
...
function groupObjects<T>(items: T[], key: (item: T) => string) {
  return items.reduce<Record<string, T[]>>((a, x) => {
    (a[key(x)] ??= []).push(x);
    return a;
  }, {});
}
...
function aggregate(
  ...
  key: (s: StudentAnalyticsRow) => string,
  label: (s: StudentAnalyticsRow) => string,
): Agg[] {
  return Object.entries(groupObjects(items, key))
    .map(([k, g]) => ({
      key: k,
      label: label(g[0]),
      students: g.length,
...
function aggregateTrend(points: TrendPoint[], mode: "daily" | "weekly" | "monthly") {
  if (mode === "daily") return points.map((p) => ({ ...p, label: p.date.slice(5) }));
  const grouped = groupObjects(points, (p) => {
    const d = new Date(`${p.date}T00:00:00`);
    if (mode === "monthly") return p.date.slice(0, 7);
    const start = new Date(d);
    start.setDate(d.getDate() - ((d.getDay() + 6) % 7));
    return iso(start);
  });
  return Object.entries(grouped).map(([label, g]) => {
    const total = g.reduce((n, p) => n + p.total, 0),
      attended = g.reduce((n, p) => n + p.attended, 0);
    return {
      label: mode === "monthly" ? label : `W ${label.slice(5)}`,
      percentage: total ? round((attended * 100) / total) : 0,
    };
  });
}
