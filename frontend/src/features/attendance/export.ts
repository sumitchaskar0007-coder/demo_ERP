...
export async function exportAttendancePdf(report: AttendanceReport, collegeName?: string) {
  ...
  document.setFontSize(16); document.setFont("helvetica", "bold"); document.text(collegeName || "Jadhavar Group of Institutes", 14, 14);
  document.setFontSize(13); document.text("Attendance Report", 14, 21);
  document.setFont("helvetica", "normal"); document.setFontSize(9); document.text(`${report.from} to ${report.to}  |  Overall: ${report.percentage}%  |  ${report.rows.length} records`, 14, 27);
  autoTable(document, { startY: 32, head: [headers], body: rows(report), styles: { fontSize: 7 }, headStyles: { fillColor: [37, 99, 235] }, didDrawPage: data => { document.setFontSize(8); document.text(`${collegeName || "Jadhavar Group of Institutes"} · Attendance Report`, 14, document.internal.pageSize.height - 7); document.text(`Page ${data.pageNumber}`, document.internal.pageSize.width - 24, document.internal.pageSize.height - 7); } });
...
export async function exportAttendanceExcel(report: AttendanceReport, collegeName?: string) {
  await exportCollegeExcel({
    ...
    metadata: [["Reporting Period", `${report.from} to ${report.to}`], ["Overall Attendance", `${report.percentage}%`], ["Total Sessions", report.sessions]],
    headers,
    rows: rows(report),
