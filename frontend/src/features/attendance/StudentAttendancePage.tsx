...
const tone = (value: number) =>
  value >= 75 ? "text-emerald-600" : value >= 60 ? "text-amber-600" : "text-rose-600";
const bar = (value: number) =>
  value >= 75 ? "bg-emerald-500" : value >= 60 ? "bg-amber-500" : "bg-rose-500";
...
export function StudentAttendancePage() {
  ...
  return (
    <div className="page-container space-y-6 pb-12">
      <header>
        <p className="text-xs font-bold uppercase tracking-widest text-brand-600">Student portal</p>
        <h1 className="page-title mt-1">My Attendance</h1>
        <p className="mt-1 text-sm text-slate-500">
          Lecture-wise attendance from submitted sessions.
        </p>
      </header>
      <div className="grid gap-4 lg:grid-cols-[1.1fr_1.9fr]">
        <Card className="relative overflow-hidden border-0 bg-slate-950 p-6 text-white">
          <div className="absolute -right-12 -top-12 h-40 w-40 rounded-full bg-brand-500/20" />
          ...
          <div className="mt-5 h-2 overflow-hidden rounded-full bg-white/10">
            <div
              className={cn("h-full rounded-full", bar(data.overallPercentage))}
              style={{ width: `${Math.min(100, data.overallPercentage)}%` }}
            />
          </div>
        </Card>
        <Card className="p-6">
          <h2 className="font-bold">Attendance guidance</h2>
      ...
      <Card className="overflow-hidden">
        ...
        {data.subjects.length === 0 ? (
          ...
          <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
            {data.subjects.map((s) => (
              <div key={s.subjectId} className="rounded-2xl border p-5">
                <div className="flex justify-between gap-3">
                  <div>
                    <h3 className="font-bold">{s.subject}</h3>
                    <p className="mt-1 text-xs text-slate-500">
                      {s.attended} attended of {s.total} lectures
                    </p>
                  </div>
                  <span className={cn("text-xl font-black", tone(s.percentage))}>
                    {s.percentage}%
                  </span>
                </div>
                <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100">
                  <div
                    className={cn("h-full rounded-full", bar(s.percentage))}
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase text-slate-500">
                <tr>
                  {["Date", "Time", "Subject", "Teacher", "Status", "Remarks"].map((h) => (
              ...
              <tbody>
                {data.history.map((r, i) => (
                  <tr key={`${r.date}-${r.time}-${i}`} className="border-t">
...
function Empty() {
  return (
    <div className="p-12 text-center text-sm text-slate-500">
      No submitted attendance is available yet.
    </div>
  );
}
