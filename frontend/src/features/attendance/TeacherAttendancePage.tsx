...
export function TeacherAttendancePage() {
  ...
  const visible = useMemo(
    () =>
      roster?.students.filter((s) =>
        `${s.studentName} ${s.rollNumber} ${s.admissionNumber}`
          .toLowerCase()
  ...
  const updateStudent = (
    studentId: number,
    patch: Partial<{ status: AttendanceStatus; remarks: string }>,
  ) =>
    setRoster(
      (current) =>
        current && {
  ...
  async function save(submit: boolean) {
    ...
    setBusy(true);
    try {
      const payload = roster.students.map(({ studentId, status, remarks }) => ({
        studentId,
        status,
        remarks,
      }));
        ...
        <div className="flex rounded-xl border bg-white p-1 shadow-sm">
          <button
            ...
            className={cn(
              ...
              tab === "current" && "bg-brand-600 text-white",
            )}
          >
          ...
          <button
            ...
            className={cn(
              "rounded-lg px-4 py-2 text-sm font-semibold",
              tab === "history" && "bg-brand-600 text-white",
            )}
          >
        ...
        <>
          ...
          <Card className="overflow-hidden border-0 bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-lg">
            <div className="grid gap-5 p-6 md:grid-cols-[1fr_auto] md:items-center">
              <div>
                <div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-wide text-blue-100">
                  <span>{lecture.period}</span>
                  <span>•</span>
                  <span>{lecture.lectureType}</span>
                  <span className="rounded-full bg-white/15 px-2.5 py-1">
                    {lecture.sessionStatus || "Not started"}
                  </span>
                </div>
                <h2 className="mt-3 text-2xl font-bold">{lecture.subject}</h2>
                <p className="mt-1 text-blue-100">
                  {lecture.year} - {lecture.division} · {lecture.department}
          ...
          {!roster ? (
            ...
            <Card className="overflow-hidden">
              <div className="border-b p-5">
                <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {roster.totalStudents} active students · default status is Present
                    </p>
                  </div>
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-4">
                  {statuses.map((status) => (
                    <div
                      key={status}
                      className={cn("rounded-xl border px-4 py-3", statusStyles[status])}
                    >
                      <p className="text-xs font-bold">{status}</p>
                      <p className="text-xl font-black">{counts[status]}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {visible.map((student) => (
                    <div
                      ...
                      className="rounded-xl border p-4 transition hover:border-brand-200 hover:shadow-sm"
                    >
                      <div className="grid gap-4 lg:grid-cols-[minmax(220px,1fr)_auto_minmax(180px,0.7fr)] lg:items-center">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {statuses.map((status) => (
                            <button
                              key={status}
                              disabled={!roster.editable}
                              onClick={() => updateStudent(student.studentId, { status })}
                              className={cn(
                                "rounded-lg border px-2.5 py-2 text-[11px] font-bold transition",
                                student.status === status
                                  ? statusStyles[status]
                                  : "border-slate-200 text-slate-500 hover:bg-slate-50",
                                !roster.editable && "cursor-not-allowed opacity-70",
                              )}
                            >
                              {student.status === status && (
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateStudent(student.studentId, { remarks: e.target.value })
                          }
                        />
                      </div>
                    </div>
              ...
              <div className="sticky bottom-0 flex flex-wrap items-center justify-between gap-3 border-t bg-white/95 p-5 backdrop-blur">
                <p className="text-xs text-slate-500">Submitted attendance cannot be changed.</p>
                {roster.editable ? (
          ...
          <Card className="w-full max-w-lg p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Review the totals. This action permanently locks the session.
                </p>
              </div>
...
function HistoryTable({ rows }: { rows: SessionSummary[] }) {
  return (
    <Card className="overflow-hidden">
      <div className="border-b p-5">
        <h2 className="font-bold">My attendance history</h2>
        <p className="text-xs text-slate-500">Only lectures conducted by you are shown.</p>
      </div>
      {rows.length === 0 ? (
        <div className="p-12 text-center text-sm text-slate-500">No attendance sessions found.</div>
      ) : (
