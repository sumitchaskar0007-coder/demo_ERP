...
export function CreatePrincipalPage() {
  ...
  const submit = async (values: FormValues) => { try { const response = await createPrincipal(values); toast.success(response.message); navigate(`/users/${response.data.id}`); } catch (error) { toast.error(handleApiError(error).message); } };
