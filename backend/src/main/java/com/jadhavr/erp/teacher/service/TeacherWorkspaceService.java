package com.jadhavr.erp.teacher.service;
...
import com.jadhavr.erp.teacher.dto.TeacherWorkspaceDtos.*;
import com.jadhavr.erp.teacher.entity.TeacherNotification;
...
public class TeacherWorkspaceService {
    ...
    private final SubjectTeacherAssignmentRepository assignments;private final WeeklyTimetableEntryRepository entries;
    ...
    public TeacherWorkspaceService(StaffProfileRepository staff,SectionRepository sections,StudentSectionEnrollmentRepository enrollments,
            SubjectTeacherAssignmentRepository assignments,WeeklyTimetableEntryRepository entries,WeeklyAttendanceSessionRepository sessions,
            WeeklyAttendanceRecordRepository records,NoticeService notices,TeacherNotificationService notificationService,StudentProfileRepository students){this.staff=staff;this.sections=sections;this.enrollments=enrollments;this.assignments=assignments;this.entries=entries;this.sessions=sessions;this.records=records;this.notices=notices;this.notificationService=notificationService;this.students=students;}
    ...
    public Workspace workspace(String search,Integer attendanceBelow,String gender,Long divisionId,int page,int size){
        ...
        List<WeeklyTimetableEntry> teacherEntries=entries.findByTeacherId(c.teacher().getId()).stream().filter(e->e.getTimetable().getStatus()==WeeklyTimetable.Status.ACTIVE).toList();
        ...
        long todayLectures=teacherEntries.stream().filter(e->e.getDayOfWeek()==today.getDayOfWeek()).count();
        ...
        assignments.findBySubjectDepartmentIdAndStatus(c.teacher().getDepartment().getId(),AcademicStatus.ACTIVE).stream().filter(a->a.getTeacher().getId().equals(c.teacher().getId())).forEach(a->subjectIds.add(a.getSubject().getId()));
        ...
        return new Workspace(c.teacher().getFullName(),c.teacher().getEmployeeCode(),c.classTeacher(),SecurityUtils.hasRole("SUBJECT_TEACHER")||!subjectIds.isEmpty(),kpis,
                ...
                attention(allStudentRows,byStudent,today),coverage(c,teacherEntries,scopedSessions),workload(c,teacherEntries,today,pending),schedule(teacherEntries,today,todayTeacher),
    ...
    private Context context(){StaffProfile teacher=staff.findByUserId(SecurityUtils.getCurrentUserId()).orElseThrow(()->new AccessDeniedException("Teacher profile not found"));if(!(SecurityUtils.hasRole("CLASS_TEACHER")||SecurityUtils.hasRole("SUBJECT_TEACHER")))throw new AccessDeniedException("Teacher dashboard access denied");List<Section> classSections=sections.findByClassTeacherIdAndStatus(teacher.getId(),SectionStatus.ACTIVE);Set<Section> visible=new LinkedHashSet<>(classSections);entries.findByTeacherId(teacher.getId()).stream().filter(e->e.getTimetable().getStatus()==WeeklyTimetable.Status.ACTIVE).map(e->e.getTimetable().getSection()).forEach(visible::add);if(teacher.getDepartment()!=null)assignments.findBySubjectDepartmentIdAndStatus(teacher.getDepartment().getId(),AcademicStatus.ACTIVE).stream().filter(a->a.getTeacher().getId().equals(teacher.getId())).flatMap(a->a.getSections().stream()).forEach(visible::add);visible.removeIf(s->!s.getCollege().getId().equals(teacher.getCollege().getId())||!s.getDepartment().getId().equals(teacher.getDepartment().getId()));return new Context(teacher,!classSections.isEmpty(),classSections,List.copyOf(visible));}
    ...
    private List<CoverageRow> coverage(Context c,List<WeeklyTimetableEntry> teacherEntries,List<WeeklyAttendanceSession> scopedSessions){Map<String,WeeklyTimetableEntry> pairs=teacherEntries.stream().collect(Collectors.toMap(e->e.getSubject().getId()+":"+e.getTimetable().getSection().getId(),Function.identity(),(a,b)->a,LinkedHashMap::new));return pairs.values().stream().map(e->{long planned=Math.max(1,Optional.ofNullable(e.getSubject().getCredits()).orElse(1))*16L;long done=scopedSessions.stream().filter(s->s.getTeacher().getId().equals(c.teacher().getId())&&s.getSubject().getId().equals(e.getSubject().getId())&&s.getSection().getId().equals(e.getTimetable().getSection().getId())&&s.getStatus()==WeeklyAttendanceSession.Status.SUBMITTED).count();double pct=Math.min(100,done*100.0/planned);String status=done>=planned?"COMPLETED":pct>=60?"ON_TRACK":"BEHIND_SCHEDULE";return new CoverageRow(e.getSubject().getId(),e.getSubject().getName(),e.getTimetable().getSection().getName(),planned,done,Math.max(0,planned-done),round(pct),status);}).toList();}
    private Workload workload(Context c,List<WeeklyTimetableEntry> teacherEntries,LocalDate today,long pending){List<WorkloadPoint> week=Arrays.stream(DayOfWeek.values()).limit(6).map(d->new WorkloadPoint(title(d.name()),teacherEntries.stream().filter(e->e.getDayOfWeek()==d).count())).toList();List<WorkloadPoint> subject=teacherEntries.stream().collect(Collectors.groupingBy(e->e.getSubject().getName(),LinkedHashMap::new,Collectors.counting())).entrySet().stream().map(e->new WorkloadPoint(e.getKey(),e.getValue())).toList();return new Workload(teacherEntries.stream().map(e->e.getSubject().getId()).distinct().count(),c.sections().size(),teacherEntries.size(),teacherEntries.stream().filter(e->e.getDayOfWeek()==today.getDayOfWeek()).count(),Math.max(0,pending),week,subject);}
    private List<ScheduleRow> schedule(List<WeeklyTimetableEntry> teacherEntries,LocalDate today,List<WeeklyAttendanceSession> todaySessions){LocalTime now=LocalTime.now();Map<Long,WeeklyAttendanceSession> byEntry=todaySessions.stream().collect(Collectors.toMap(s->s.getTimetableEntry().getId(),Function.identity(),(a,b)->a));return teacherEntries.stream().filter(e->e.getDayOfWeek()==today.getDayOfWeek()).sorted(Comparator.comparing(e->e.getPeriod().getStartTime())).map(e->{String state=now.isAfter(e.getPeriod().getEndTime())?"COMPLETED":!now.isBefore(e.getPeriod().getStartTime())?"CURRENT":"UPCOMING";WeeklyAttendanceSession s=byEntry.get(e.getId());return new ScheduleRow(e.getId(),e.getPeriod().getStartTime()+" - "+e.getPeriod().getEndTime(),e.getSubject().getName(),e.getTimetable().getSection().getAcademicClass().getName()+" "+e.getTimetable().getSection().getName(),e.getLectureType().name(),state,s==null?null:s.getId(),"CURRENT".equals(state)&&(s==null||s.getStatus()==WeeklyAttendanceSession.Status.DRAFT));}).toList();}
