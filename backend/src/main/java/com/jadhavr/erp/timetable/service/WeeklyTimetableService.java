...
public class WeeklyTimetableService {
    ...
    private void requireViewer(){if(!(SecurityUtils.hasRole("PRINCIPAL")||SecurityUtils.hasRole("HOD")))throw new AccessDeniedException("Weekly timetable access denied");}
    ...
    private boolean visible(Section s){requireViewer();var u=SecurityUtils.requireCurrentUser();if(!Objects.equals(u.getCollegeId(),s.getCollege().getId()))return false;if(SecurityUtils.hasRole("HOD"))return staff.findByUserId(u.getId()).map(x->x.belongsToDepartment(s.getDepartment().getId())).orElse(false);return SecurityUtils.hasRole("PRINCIPAL");}
