...
public class WeeklyTimetableService {
    ...
    private boolean editable(Section s){if(SecurityUtils.isSuperAdmin())return false;if(SecurityUtils.hasRole("PRINCIPAL"))return true;if(!SecurityUtils.hasRole("CLASS_TEACHER"))return false;return staff.findByUserId(SecurityUtils.getCurrentUserId()).map(x->s.getClassTeacher()!=null&&s.getClassTeacher().getId().equals(x.getId())).orElse(false);}
