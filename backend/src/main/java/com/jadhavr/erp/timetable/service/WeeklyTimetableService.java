...
public class WeeklyTimetableService {
    ...
    private void requireViewer(){if(!(SecurityUtils.hasRole("PRINCIPAL")||SecurityUtils.hasRole("HOD")||SecurityUtils.hasRole("CLASS_TEACHER")))throw new AccessDeniedException("Weekly timetable access denied");}
    ...
    private boolean visible(Section s){requireViewer();var u=SecurityUtils.requireCurrentUser();if(!Objects.equals(u.getCollegeId(),s.getCollege().getId()))return false;Optional<StaffProfile> profile=staff.findByUserId(u.getId());if(SecurityUtils.hasRole("HOD"))return profile.map(x->x.belongsToDepartment(s.getDepartment().getId())).orElse(false);if(SecurityUtils.hasRole("CLASS_TEACHER"))return profile.map(x->s.getClassTeacher()!=null&&s.getClassTeacher().getId().equals(x.getId())).orElse(false);return SecurityUtils.hasRole("PRINCIPAL");}
