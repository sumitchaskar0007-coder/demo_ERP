...
public class HodModuleService {
    ...
    public WorkspaceResponse workspace(Long requestedDepartmentId,String search,Long courseYearId,Long divisionId,
        ...
        List<WeeklyTimetable> tableRows=timetables.findBySectionDepartmentId(departmentId);
        Summary summary=summary(departmentId,teachers,divisionRows,tableRows,allStudents.size());
    ...
    @Transactional(readOnly=true)
    public List<RollPreview> previewRolls(RollPreviewRequest request){Section section=section(request.sectionId());List<StudentSectionEnrollment> rows=new ArrayList<>(enrollments.findBySectionIdAndStatus(section.getId(),AcademicStatus.ACTIVE));Comparator<StudentSectionEnrollment> comparator=switch(request.strategy().toUpperCase(Locale.ROOT)){case "ADMISSION_DATE"->Comparator.comparing(e->Optional.ofNullable(e.getStudent().getActivatedAt()).orElse(LocalDateTime.MAX));case "ADMISSION_NUMBER","MERIT_RANK"->Comparator.comparing(e->e.getStudent().getAdmissionNumber(),String.CASE_INSENSITIVE_ORDER);default->Comparator.comparing(e->e.getStudent().getFullName(),String.CASE_INSENSITIVE_ORDER);};rows.sort(comparator.thenComparing(e->e.getStudent().getId()));int[] n={1};return rows.stream().map(e->new RollPreview(e.getStudent().getId(),e.getStudent().getFullName(),e.getStudent().getAdmissionNumber(),pending(e.getStudent().getRollNumber())?null:e.getStudent().getRollNumber(),section.getCode()+"-"+String.format("%03d",n[0]++))).toList();}

    public int confirmRolls(RollConfirmRequest request){Section section=section(request.sectionId());Map<Long,StudentSectionEnrollment> owned=enrollments.findBySectionIdAndStatus(section.getId(),AcademicStatus.ACTIVE).stream().collect(Collectors.toMap(e->e.getStudent().getId(),Function.identity()));Set<String> batch=new HashSet<>();for(RollAssignment item:request.assignments()){String roll=item.rollNumber().trim();if(!batch.add(roll.toLowerCase(Locale.ROOT)))throw new BadRequestException("Roll numbers must be unique");StudentSectionEnrollment e=owned.get(item.studentId());if(e==null)throw new BadRequestException("Student does not belong to this division");String current=e.getStudent().getRollNumber();if(!request.overwrite()&&!pending(current)&&!current.equalsIgnoreCase(roll))throw new BadRequestException(e.getStudent().getFullName()+" already has a roll number");if(students.existsByRollNumber(roll)&&!roll.equalsIgnoreCase(current))throw new BadRequestException("Roll number "+roll+" already exists");}for(RollAssignment item:request.assignments()){StudentSectionEnrollment e=owned.get(item.studentId());String roll=item.rollNumber().trim();e.setRollNumber(roll);e.getStudent().setRollNumber(roll);students.save(e.getStudent());enrollments.save(e);}notifications.notifyDivision(section,"ROLL_NUMBERS_GENERATED","Roll numbers generated for "+section.getName());audit.log(AuditModule.ACADEMIC,AuditAction.UPDATE,"Section",section.getId(),"Generated "+request.assignments().size()+" roll numbers for "+section.getName());return request.assignments().size();}

    public void allocateSubject(Long subjectId,SubjectAllocationRequest request){Subject subject=subject(subjectId);StaffProfile teacher=teacher(request.teacherId(),subject.getDepartment().getId());List<Section> divisions=request.divisionIds().stream().distinct().map(this::section).toList();for(Section s:divisions)if(!s.getAcademicClass().getId().equals(subject.getAcademicClass().getId())||!s.getAcademicYear().equals(subject.getAcademicYear()))throw new BadRequestException("Every division must match the subject course and academic year");long existing=timetableEntries.findByTeacherId(teacher.getId()).size();long requested=Math.max(1,Optional.ofNullable(subject.getCredits()).orElse(1))*divisions.size();if(existing+requested>MAX_WEEKLY_LECTURES)throw new BadRequestException("Assignment exceeds the "+MAX_WEEKLY_LECTURES+" lecture weekly workload limit");SubjectTeacherAssignment row=subjectAssignments.findBySubjectIdAndTeacherIdAndStatus(subjectId,teacher.getId(),AcademicStatus.ACTIVE).orElseGet(SubjectTeacherAssignment::new);row.setSubject(subject);row.setTeacher(teacher);row.setAcademicYear(subject.getAcademicYear());row.setSections(divisions);subjectAssignments.save(row);notifications.notifyTeacher(teacher,"TEACHING_ASSIGNMENT_UPDATED","You are assigned to "+subject.getName()+" for "+divisions.size()+" division(s)");audit.log(AuditModule.ACADEMIC,AuditAction.ASSIGN,"Subject",subjectId,"Assigned "+teacher.getFullName()+" to "+subject.getName()+" for "+divisions.size()+" divisions");}
