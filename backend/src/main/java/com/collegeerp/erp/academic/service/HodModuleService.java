...
import com.collegeerp.erp.audit.enums.*;
...
import com.collegeerp.erp.staff.enums.*;
...
import com.collegeerp.erp.timetable.repository.*;
...
public class HodModuleService {
    ...
    private static final Set<StaffType> TEACHING_TYPES=EnumSet.of(StaffType.HOD,StaffType.TEACHER,StaffType.CLASS_TEACHER,StaffType.SUBJECT_TEACHER);
    ...
    private final AcademicSessionResolver sessionResolver;
    ...
    public HodModuleService(StudentProfileRepository students,StaffProfileRepository staff,SectionRepository sections,
            ...
            AuthorizationSnapshotService authorizationSnapshots,AcademicSessionResolver sessionResolver){
    ...
    public WorkspaceResponse workspace(Long requestedDepartmentId,String search,Long courseYearId,Long divisionId,
        ...
        List<StaffProfile> teachers=staff.findTeachingByCollegeAndDepartment(
                scope.collegeId(),departmentId,StaffStatus.ACTIVE,TEACHING_TYPES);
        ...
        List<WeeklyTimetable> activeTableRows=timetables.findBySectionDepartmentIdAndSemesterOfferingStatusAndStatus(departmentId,SemesterOfferingStatus.ACTIVE,WeeklyTimetable.Status.ACTIVE);
        Map<Long,WeeklyTimetable> workingTables=timetables.findBySectionDepartmentIdAndSemesterOfferingStatusAndStatusNot(
                        departmentId,SemesterOfferingStatus.ACTIVE,WeeklyTimetable.Status.ARCHIVED).stream()
        ...
        return new WorkspaceResponse(departmentId,scope.departmentName(),summary,
                ...
                subjects.findByDepartmentIdAndStatus(departmentId,SubjectStatus.ACTIVE).stream().map(s->subjectRow(s,assignments.getOrDefault(s.getId(),List.of()))).toList(),
                tableRows.stream().map(this::timetableRow).toList(),activity);
    }

    @Transactional(readOnly=true)
    public Summary dashboard(Long requestedDepartmentId){Scope s=scope(requestedDepartmentId);List<Section>d=sections.findByDepartmentIdAndStatus(s.departmentId(),SectionStatus.ACTIVE);List<StaffProfile>t=staff.findTeachingByCollegeAndDepartment(s.collegeId(),s.departmentId(),StaffStatus.ACTIVE,TEACHING_TYPES);long studentCount=students.countByDepartmentIdAndStatus(s.departmentId(),StudentStatus.ACTIVE);return summary(s.departmentId(),t,d,timetables.findBySectionDepartmentIdAndSemesterOfferingStatusAndStatus(s.departmentId(),SemesterOfferingStatus.ACTIVE,WeeklyTimetable.Status.ACTIVE),studentCount);}
    ...
    private TimetableReviewRow timetableRow(WeeklyTimetable t){Section s=t.getSection();return new TimetableReviewRow(t.getId(),s.getId(),s.getName(),s.getAcademicClass().getName(),s.getAcademicYear(),t.getReviewStatus().name(),t.getReviewComment(),t.getSubmittedAt(),t.getReviewedAt(),timetableEntries.countByTimetableId(t.getId()));}
    private Summary summary(Long departmentId,List<StaffProfile> teachers,List<Section> divisions,List<WeeklyTimetable> tables,long studentCount){long subjectCount=subjects.countByDepartmentIdAndStatus(departmentId,SubjectStatus.ACTIVE);DayOfWeek today=LocalDate.now().getDayOfWeek();long classes=timetableEntries.countDistinctTimetablesByDepartmentAndDay(departmentId,WeeklyTimetable.Status.ACTIVE,WeeklyTimetable.ReviewStatus.APPROVED,today);long totalAttendance=attendance.countBySessionSectionDepartmentIdAndSessionStatus(departmentId,WeeklyAttendanceSession.Status.SUBMITTED);long attended=attendance.countBySessionSectionDepartmentIdAndSessionStatusAndStatusIn(departmentId,WeeklyAttendanceSession.Status.SUBMITTED,List.of(WeeklyAttendanceRecord.Status.PRESENT,WeeklyAttendanceRecord.Status.LATE));double average=totalAttendance==0?0:attended*100.0/totalAttendance;long allocated=enrollments.countBySectionDepartmentIdAndStatus(departmentId,AcademicStatus.ACTIVE);long pending=Math.max(0,studentCount-allocated)+tables.stream().filter(t->t.getReviewStatus()==WeeklyTimetable.ReviewStatus.SUBMITTED).count();return new Summary(studentCount,teachers.size(),divisions.size(),subjectCount,classes,Math.round(average*10)/10.0,pending);}
    ...
    private StaffProfile teacher(Long id,Long departmentId){StaffProfile t=staff.findById(id).orElseThrow(()->new ResourceNotFoundException("Teacher not found"));if(t.getStatus()!=StaffStatus.ACTIVE||!TEACHING_TYPES.contains(t.getStaffType())||!t.canTeachInDepartment(departmentId))throw new BadRequestException("Teacher is not eligible to teach in this department");return t;}
    private boolean canBecomeClassTeacher(StaffProfile teacher){
        return TEACHING_TYPES.contains(teacher.getStaffType())
