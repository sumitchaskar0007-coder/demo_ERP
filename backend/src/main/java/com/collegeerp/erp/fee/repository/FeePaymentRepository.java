...
public interface FeePaymentRepository extends JpaRepository<FeePayment, Long>,
    ...
    @Query("""
            ...
            from FeePayment p
            where p.status = com.collegeerp.erp.fee.enums.PaymentStatus.VERIFIED
    ...
    @Query(value = """
            select new com.collegeerp.erp.fee.dto.FeeCollectionRow(
                p.id,
                p.student.fullName,
                p.college.name,
                p.department.name,
            ...
            from FeePayment p
            left join StudentSectionEnrollment e on e.student = p.student and e.status = com.collegeerp.erp.academic.enums.AcademicStatus.ACTIVE and e.academicYear = p.studentFeeAccount.academicYear
    ...
    @Query("""
            ...
            where p.status = com.collegeerp.erp.fee.enums.PaymentStatus.VERIFIED
              ...
              and (:divisionId is null or division.id = :divisionId)
            """)
    BigDecimal sumVerifiedCollections(
            @Param("collegeId") Long collegeId,
            @Param("departmentId") Long departmentId,
            @Param("academicYear") String academicYear,
            @Param("studentCategory") StudentCategory studentCategory,
