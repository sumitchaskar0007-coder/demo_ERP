...
import com.collegeerp.erp.academic.mapper.DivisionMapper;
...
import com.collegeerp.erp.staff.mapper.StaffMapper;
import com.collegeerp.erp.staff.repository.StaffProfileRepository;
import com.collegeerp.erp.user.entity.*;
...
class DivisionServiceImplTest {
    ...
    @Test void eligibleClassTeachersExcludeTeachersWithAnActiveDivision(){Section division=division();StaffProfile teacher=teacher(5L,department(2L));when(divisions.findById(4L)).thenReturn(Optional.of(division));when(staff.findByCollegeId(1L)).thenReturn(java.util.List.of(teacher));when(divisions.findByClassTeacherIdAndStatus(5L,SectionStatus.ACTIVE)).thenReturn(java.util.List.of(division));assertTrue(service.eligibleClassTeachers(4L).isEmpty());}
    @Test void hodIsEligibleAsClassTeacherAcrossCollegeDepartments(){Section division=division();StaffProfile hod=teacher(5L,department(99L));hod.setStaffType(StaffType.HOD);hod.getUser().setRoles(Set.of(role(RoleName.HOD)));when(divisions.findById(4L)).thenReturn(Optional.of(division));when(staff.findByCollegeId(1L)).thenReturn(java.util.List.of(hod));when(divisions.findByClassTeacherIdAndStatus(5L,SectionStatus.ACTIVE)).thenReturn(java.util.List.of());assertEquals(1,service.eligibleClassTeachers(4L).size());}
    @Test void removingLeadershipClassTeacherPreservesLeadershipRole(){Section division=division();StaffProfile hod=teacher(5L,department(2L));hod.setStaffType(StaffType.HOD);hod.getUser().setRoles(Set.of(role(RoleName.HOD),role(RoleName.CLASS_TEACHER)));division.setClassTeacher(hod);when(divisions.findById(4L)).thenReturn(Optional.of(division));when(divisions.save(division)).thenReturn(division);when(divisions.findByClassTeacherIdAndStatus(5L,SectionStatus.ACTIVE)).thenReturn(java.util.List.of());service.removeClassTeacher(4L);assertTrue(hod.getUser().getRoles().stream().anyMatch(r->r.getName()==RoleName.HOD));assertFalse(hod.getUser().getRoles().stream().anyMatch(r->r.getName()==RoleName.CLASS_TEACHER));}
